            ...
            <div class="mt-10 sm:mt-0 w-full">
                <div class="md:grid md:gap-6">
                    ...
                    <div class="mt-5 md:mt-0 md:col-span-2">
                        <ul class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-md">
                            @foreach ($categories as $category)
                            <li class="flex justify-between w-full border-b-2 py-2">
                                <span class="item">
                                    {{ $category->name }}
                                </span>
                                <div class="actions flex gap-4">
                                    <a class="inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring focus:ring-blue-300 disabled:opacity-25 transition" href="{{route('category.edit', ['id'=>$category->id])}}">edit</a>
                                    <a class="inline-flex items-center px-4 py-2 bg-red-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring focus:ring-red-300 disabled:opacity-25 transition" href="{{route('category.delete', ['id'=>$category->id])}}">delete</a>                                    
                                </div>
                            </li>
                            @endforeach
                        </ul>
